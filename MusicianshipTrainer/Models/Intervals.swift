...
class Intervals : ObservableObject {
    ...
    init(grade:Int, questionType:QuestionType) {
        ...
        if grade >= 1 {
            ...
            intervalNames.append(IntervalGroup(name: ageGroup == Settings.AgeGroup11Plus ? "Second" : "2nd", noteSpan: 1, intervals:[1,2,3]))
            intervalNames.append(IntervalGroup(name: ageGroup == Settings.AgeGroup11Plus ? "Third" : "3rd", noteSpan: 2, intervals:[3,4]))
        }
        ...
        if grade >= 2 {
            ...
            intervalNames.append(IntervalGroup(name: ageGroup == Settings.AgeGroup11Plus ? "Fourth" : "4th", noteSpan: 3, intervals:[5,6]))
            
            ///Diminished, perfect and augmented 5ths
            intervalNames.append(IntervalGroup(name: ageGroup == Settings.AgeGroup11Plus ? "Fifth" : "5th", noteSpan: 4, intervals:[6,7,8]))
        ...
        if grade >= 3 && questionType == .intervalVisual {
            ...
            intervalNames.append(IntervalGroup(name: ageGroup == Settings.AgeGroup11Plus ? "Sixth" : "6th", noteSpan: 5, intervals:[8,9,10]))
            intervalNames.append(IntervalGroup(name: ageGroup == Settings.AgeGroup11Plus ? "Seventh" : "7th", noteSpan: 6, intervals:[10,11]))
            intervalNames.append(IntervalGroup(name: ageGroup == Settings.AgeGroup11Plus ? "Octave" : "Octave", noteSpan: 7, intervals:[12]))
        }
